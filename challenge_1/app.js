...
var headerRender = function(input) {
  if (Object.keys(input)[0] === "lastWinner") {
    ...
    header[0].innerHTML = `<h1 style="color: red">Player ${board.players.key[
      input.lastWinner
    ].toUpperCase()} You're up first because youre awesome!</h1>`;
  }
...
var squareListner = function(board, off) { //game action really happens here.
  if (off) {
    ...
    squareNodes.map(elem => {
      elem.onclick = function(e) {
        ...
        elem.innerHTML = `<img class="selected-${playerLetter}" src = ${
          players.src[currentPlayer]
        } / >`;
